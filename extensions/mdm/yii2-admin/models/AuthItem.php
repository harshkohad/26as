...
class AuthItem extends Model {
    ...
    public function getItems() {
        ...
        foreach (array_keys($manager->getPermissions()) as $name) {
            if ($name === $routeDetails->name) {
                if (in_array($name, $currentUserRoutes)) {
                    $avaliable[$name] = $name[0] == '/' ? 'route' : 'permission';
                }
            } else {
                if ($name[0] == '/') {
                    $avaliable[$name] = 'route';
                } else {
                    $premissionDetails = $manager->getPermission($name);
                    $childFlag = $manager->hasChild($premissionDetails, $routeDetails);
                    if ($childFlag) {
                        if (in_array($routeDetails->name, $currentUserRoutes)) {
