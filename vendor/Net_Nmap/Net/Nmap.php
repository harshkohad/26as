...
class Net_Nmap {
    ...
    public function __construct(array $options = array()) {
        ...
        if (array_key_exists('input_file', $options)) {
            $this->_input_file = (string) $options['input_file'];
        }
    ...
    protected function createCommandLine($targets) {
        ...
        if ($this->_input_file === null) {
            $this->_input_file = $this->_output_file . '_targets';
        }
        $targetsContent = '';
        foreach ($targets as $target) {
            $targetsContent .= $target . ' ';
        }
        file_put_contents($this->_input_file, $targetsContent);
        $cmd = escapeshellarg($this->_nmap_binary);
        $cmd .= ' -v';
        $cmd .= ' ' . implode(' ', $this->_nmap_options);
        $cmd .= ' -oX ' . escapeshellarg($this->_output_file) . ' ';
        $cmd .= ' -iL ' . $this->_input_file . ' ';
        $cmd .= '2>&1';
        if (OS_WINDOWS) {
            $cmd = '"' . $cmd . '"';
        }
    ...
    public function enableOptions($nmap_options) {
        ...
        $host_discovery_options = array_key_exists('host_discovery', $nmap_options);
        ...
        if ($host_discovery_options && !empty($nmap_options['host_discovery'])) {
            $this->_nmap_options[] = $nmap_options['host_discovery'];
        }
